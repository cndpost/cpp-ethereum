...
public:
	...
	void clear();
	void noteDirty() const { Guard l(x_hash); m_hashWithout = m_hash = h256(); }
	...
	template <class T> void setSeal(unsigned _offset, T const& _value) { Guard l(x_seal); if (m_seal.size() <= _offset) m_seal.resize(_offset + 1); m_seal[_offset] = rlp(_value); noteDirty(); }
	...
	template <class T> T seal(unsigned _offset = 0) const { T ret; Guard l(x_seal); if (_offset < m_seal.size()) ret = RLP(m_seal[_offset]).convert<T>(RLP::VeryStrict); return ret; }
...
private:
	...
	mutable Mutex x_hash;			///< A lock for both m_hash and m_hashWithout.
};
